#!/usr/bin/env bash
# Testa o parser C com os casos .c e compara a saída com o resultado
# esperado.
# Uso: ./testar_parser_c_oficial.sh [diretorio_dos_testes] [codigo_do_parser]
# Exemplo: ./testar_parser_c_oficial.sh ./testes-parser-50 ./parser.c
set -u
TEST_DIR="${1:-.}"
PARSER="${2:-./parser.c}"
KEEP_TMP="${KEEP_TMP:-0}"
...
for source in "${CASES[@]}"; do
  ...
  actual="$TMP_DIR/$INDEX.actual"
  cat "$out" > "$actual"; [[ -s "$err" ]] && cat "$err" >> "$actual"
